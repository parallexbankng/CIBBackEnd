...
namespace CIB.IntraBankTransactionService.Modules.BulkCreditLog;
  ...
  public class BulkCreditLogRepository : Repository<TblNipbulkCreditLog>,IBulkCreditLogRepository
  {
      ...
      {
        return _context.TblNipbulkCreditLogs.Where(ctx => ctx.InitiateDate != null && ctx.TranLogId == tranLogId && ctx.CreditStatus == status && ctx.NameEnquiryStatus == 1 && ctx.CreditBankCode == bankCode && ctx.InitiateDate.Value.Date == processDate && ctx.TryCount <= 5).ToList();
      }
