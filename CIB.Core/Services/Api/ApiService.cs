...
namespace CIB.Core.Services.Api
{
    public class ApiService : IApiService
    {
        ...
        public async Task<CustomerDataResponseDto> GetCustomerDetailByAccountNumber(string accountNumber)
        {
            ...
            {
                var errorItem = ex.InnerException != null ? new CustomerDataResponseDto { ResponseCode = ResponseCode.API_ERROR, ResponseDescription = ex.InnerException.Message } : new CustomerDataResponseDto { ResponseCode = ResponseCode.API_ERROR, ResponseDescription = ex.InnerException != null ? ex.InnerException.Message : ex.Message };
                _logger.LogError("API ERROR {0}", Formater.JsonType(ex));
                return errorItem;
            }
        }
        public async Task<IntraBankTransferResponse> IntraBankTransfer(IntraBankPostDto transaction)
        {
            ...
            {
                var errorItem = ex.InnerException != null ? new IntraBankTransferResponse { ResponseCode = ResponseCode.API_ERROR, ResponseDescription = ex.InnerException.Message } : new IntraBankTransferResponse { ResponseCode = ResponseCode.API_ERROR, ResponseDescription = ex.InnerException != null ? ex.InnerException.Message : ex.Message };
                _logger.LogError("API ERROR {0}", Formater.JsonType(ex));
                return errorItem;
            }
        }
        public async Task<IntraBankTransferResponse> InterBankTransfer(InterBankPostDto transaction)
        {
            ...
            {
                var response = await _httpClient.PostAsync(url, payLoad).Result.Content.ReadAsStringAsync();
				_logger.LogError("InterBankTransfer RESPONSE {0}", Formater.JsonType(response));
				if (string.IsNullOrEmpty(response))
                {
        ...
        public Task<CustomerDataResponseDto> QueryTransferTransaction(string accountNumber)
        {
        ...
        public async Task<BankListResponseData> GetBanks()
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(response))
                {
                    return new BankListResponseData { ResponseCode = ResponseCode.API_ERROR, ResponseMessage = "API Error Fetch banks Failed" };
                }
                return JsonConvert.DeserializeObject<BankListResponseData>(response);
            }
            catch (Exception ex)
            {
                var errorItem = ex.InnerException != null ? new BankListResponseData { ResponseCode = ResponseCode.API_ERROR, ResponseMessage = ex.InnerException.Message } : new BankListResponseData { ResponseCode = ResponseCode.API_ERROR, ResponseMessage = ex.InnerException != null ? ex.InnerException.Message : ex.Message };
                _logger.LogError("API ERROR {0}", Formater.JsonType(ex));
                return errorItem;
            }
        }
        public async Task<InterbankNameEnquiryResponseDto> BankNameInquire(string accountNumber, string bankCode)
        {
            ...
            {
                var errorItem = ex.InnerException != null ? new InterbankNameEnquiryResponseDto { ResponseCode = ResponseCode.API_ERROR, ResponseMessage = ex.InnerException.Message } : new InterbankNameEnquiryResponseDto { ResponseCode = ResponseCode.API_ERROR, ResponseMessage = ex.InnerException != null ? ex.InnerException.Message : ex.Message };
                _logger.LogError("InterbankNameEnquiry API ERROR {0}", Formater.JsonType(ex));
                return errorItem;
            }
        }
        public async Task<RelatedCustomerAccountDetailsDto> RelatedCustomerAccountDetails(string CustomerId)
        {
            ...
            try
            {
                ...
                return JsonConvert.DeserializeObject<RelatedCustomerAccountDetailsDto>(response);
            }
            catch (Exception ex)
            {
                var errorItem = ex.InnerException != null ? new RelatedCustomerAccountDetailsDto { RespondCode = ResponseCode.API_ERROR, RespondMessage = ex.InnerException.Message } : new RelatedCustomerAccountDetailsDto { RespondCode = ResponseCode.API_ERROR, RespondMessage = ex.InnerException != null ? ex.InnerException.Message : ex.Message };
                _logger.LogError("RelatedCustomerAccountDetails API ERROR {0}", Formater.JsonType(ex));
                return errorItem;
            }
        ...
        public async Task<CustomerDataResponseDto> CustomerNameInquiry(string accountNumber)
        {
            ...
            {
                var errorItem = ex.InnerException != null ? new CustomerDataResponseDto { ResponseCode = ResponseCode.API_ERROR, ResponseDescription = ex.InnerException.Message } : new CustomerDataResponseDto { ResponseCode = ResponseCode.API_ERROR, ResponseDescription = ex.InnerException != null ? ex.InnerException.Message : ex.Message };
                _logger.LogError("CustomerNameInquiry API ERROR {0}", Formater.JsonType(ex));
                return errorItem;
            }
        }
        public async Task<StatementOfAccountResponseDto> GenerateStatement(StatementOfAccountRequestDto accountRequestDto)
        {
            ...
            {
                var errorItem = ex.InnerException != null ? new StatementOfAccountResponseDto { ResponseCode = ResponseCode.API_ERROR, ResponseDescription = ex.InnerException.Message } : new StatementOfAccountResponseDto { ResponseCode = ResponseCode.API_ERROR, ResponseDescription = ex.InnerException != null ? ex.InnerException.Message : ex.Message };
                _logger.LogError("GenerateStatement API ERROR {0}", Formater.JsonType(ex));
                return errorItem;
            }
        }
        public async Task<AdUserInfo> ADBasicInfoInquire(string UserName)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(response))
                {
                    return new AdUserInfo { ResponseCode = ResponseCode.API_ERROR, ResponseDescription = "API Error Query AD" };
                }
                return JsonConvert.DeserializeObject<AdUserInfo>(response);
            }
            catch (Exception ex)
            {
                var errorItem = ex.InnerException != null ? new AdUserInfo { ResponseCode = ResponseCode.API_ERROR, ResponseDescription = ex.InnerException.Message } : new AdUserInfo { ResponseCode = ResponseCode.API_ERROR, ResponseDescription = ex.InnerException != null ? ex.InnerException.Message : ex.Message };
                _logger.LogError("API ERROR {0}", Formater.JsonType(ex));
                return errorItem;
            }
        }
        ...
        public async Task<BulkIntraBankTransactionResponse> IntraBankBulkTransfer(BulkIntrabankTransactionModel transaction)
        {
            ...
            {
				var response = await _httpClient.PostAsync(url, payLoad).Result.Content.ReadAsStringAsync();
				_logger.LogError("IntraBankBulkTransfer Payload {0}", Formater.JsonType(response));
				if (string.IsNullOrEmpty(response))
                {
                    return new BulkIntraBankTransactionResponse { ResponseCode = ResponseCode.API_ERROR, ResponseMessage = "Api Error Processing Bulk Transaction" };
                }
            ...
            {
                var message = ex.InnerException != null ? ex.InnerException.Message : ex.Message;
                _logger.LogError("IntraBankBulkTransfer API ERROR {0}", Formater.JsonType(ex));
