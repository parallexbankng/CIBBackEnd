...
namespace CIB.Core.Services.Notification
{
	public class NotificationService : INotificationService
	{
		...
		//NOTIFY ONLENDING  AUTHORIZER FOR TRANSFER 
		public void NotifyOnlendingAuthorizerApprovalRequest(EmailNotification profile, EmailNotification notify = null)
		{
			ThreadPool.QueueUserWorkItem(_ => _emailService.SendEmail(Transfer.DeclineApproval(profile.Email, notify)));
		}

		public void NotifyOnlendingInitiatorForDeclineRequest(TblBankProfile profile, EmailNotification notify)
		{
			ThreadPool.QueueUserWorkItem(_ => _emailService.SendEmail(Transfer.DeclineApproval(profile.Email, notify)));
		}

		public void NotifyBankAdminAuthorizerForCorporateCustomerAggregationApproval(EmailNotification profile = null)
		{
			var bankAuthorization = this.GetBankAdminAuthorizer();
			foreach (var bank in bankAuthorization)
			{
