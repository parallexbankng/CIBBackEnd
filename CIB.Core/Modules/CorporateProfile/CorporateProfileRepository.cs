	...
	public class CorporateProfileRepository : Repository<TblCorporateProfile>, ICorporateProfileRepository
	{
		...
		public IEnumerable<CorporateProfileResponseDto> GetAllCorporateProfiles()
		{
			var corporateProfileModel = (
							 ...
							 join company in _context.TblCorporateCustomers on corporateProfile.CorporateCustomerId.ToString() equals company.Id.ToString()
							 select new CorporateProfileResponseDto
							 {
								 ...
								 CorporateShortName = company.CorporateShortName
							 }).ToList();
		...
		public IEnumerable<CorporateProfileResponseDto> GetSingleSignatoryCorporateProfilesByCorporateCustomerId(Guid id)
		{
			var corporateProfileModel = (
								...
								join company in _context.TblCorporateCustomers on corporateProfile.CorporateCustomerId equals company.Id
								select new CorporateProfileResponseDto
								{
									Id = corporateProfile.Id,
									AcctBalance = corporateProfile.AcctBalance,
		...
		{
			var custName = userName.Trim().ToLower();
			return _context.TblCorporateProfiles.Where(a => a.CorporateCustomerId == CorporateCustomerId && a.Username.Trim().ToLower().Equals(custName)).FirstOrDefault();
		}
		...
		public TblCorporateProfile CheckDuplicateUserName(string userName)
		{
			return _context.TblCorporateProfiles.FirstOrDefault(a => a.Username.Trim().ToLower().Equals(userName.Trim().ToLower()));
		}
		public List<TblCorporateProfile> GetProfileByCorporateCustomerId(Guid CorporateCustomerId)
		{
			return _context.TblCorporateProfiles.Where(ctx => ctx.CorporateCustomerId == CorporateCustomerId).ToList();
		}

		public TblCorporateProfile CheckActiveProfile(TblCorporateProfile activeProfile, Guid CorporateCustomerId)
		{
			return _context.TblCorporateProfiles.FirstOrDefault(ctx => ctx.CorporateCustomerId == CorporateCustomerId && ctx.Status == (int)ProfileStatus.Active);
		}
	}
}
