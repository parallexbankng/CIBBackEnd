...
namespace CIB.Core.Modules.Workflow
{
    public class WorkFlowRepository : Repository<TblWorkflow>, IWorkFlowRepository
    {
        ...
        public List<TblWorkflow> GetActiveWorkflow(Guid corporateCustomerId)
        {
            return _context.TblWorkflows.Where(x => x.CorporateCustomerId == corporateCustomerId && x.Status == (int)ProfileStatus.Active).OrderByDescending(ctx => ctx.Sn).ToList();
