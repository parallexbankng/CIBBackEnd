	...
	public class CorporateProfileController : BaseAPIController
	{
		...
		public ActionResult<ResponseDTO<CorporateProfileResponseDto>> UpdateCorporateProfile(GenericRequestDto model)
		{
			try
			{
				...
				mapTempProfile.Username = payload.Username;
		...
		public ActionResult<ResponseDTO<CorporateProfileResponseDto>> RequestProfileApproval(GenericRequestDto model)
		{
			try
			{
				...
				if (entity.Action == nameof(TempTableAction.Pin_Reset).Replace("_", " "))
				{
					if (string.IsNullOrEmpty(UserRoleId) || !UnitOfWork.UserRoleAccessRepo.AccessesExist(UserRoleId, Permission.CanInitiateResetTransactionPin))
					{
						return BadRequest("UnAuthorized Access");
					}
				}
		...
		public ActionResult<ResponseDTO<CorporateProfileResponseDto>> ApproveProfile(GenericRequestDto model)
		{
			try
			{
				...
				if (entity.Action == nameof(TempTableAction.Pin_Reset).Replace("_", " "))
				{
					if (string.IsNullOrEmpty(UserRoleId) || !UnitOfWork.UserRoleAccessRepo.AccessesExist(UserRoleId, Permission.CanApprovedTransactionPinReset))
					{
						return BadRequest("UnAuthorized Access");
					}
				}
		...
		public ActionResult<ResponseDTO<CorporateProfileResponseDto>> ReActivateProfile(GenericRequestDto model)
		{
			try
			{
				...
				if (Enum.TryParse(corporateCustomerDto.AuthorizationType.Replace(" ", "_"), out AuthorizationType _auth))
				{
					...
					if (_auth == AuthorizationType.Single_Signatory)
					{
						var checkForActiveUser = UnitOfWork.CorporateProfileRepo.CheckActiveProfile(entity, corporateCustomerDto.Id);
						if (checkForActiveUser != null)
						{
							return BadRequest("only one active user is allow on single signatory");
						}
					}
				}
		...
		[ProducesResponseType(StatusCodes.Status201Created)]
		public ActionResult<ResponseDTO<CorporateProfileResponseDto>> UpdateProfileUserName(GenericRequestDto model)
		{
			try
			{
				if (!IsAuthenticated)
				...
				var requestData = JsonConvert.DeserializeObject<UpdateProfileUserNameDTO>(Encryption.DecryptStrings(model.Data));
				if (requestData == null)
				{
					return BadRequest("invalid request data");
				}
				...
				var payload = new UpdateProfileUserNameDTO
				{
					Id = requestData.Id,
					UserName = requestData.UserName,
					IPAddress = Encryption.DecryptStrings(model.IPAddress),
				...
				//get profile by id
				var validator = new UpdateCorporateProfileUserNameValidation();
				var results = validator.Validate(payload);
				if (!results.IsValid)
				{
					return UnprocessableEntity(new ValidatorResponse(_data: new object(), _success: false, _validationResult: results.Errors));
				...
				// check for duplicate
				var checkDuplicateUserName = UnitOfWork.CorporateProfileRepo.CheckDuplicateUserName(payload.UserName);
				if (checkDuplicateUserName != null)
				{
					if (checkDuplicateUserName.Status == (int)ProfileStatus.Pending)
					{
						return BadRequest("profile with this user name is awaiting approval");
					}
					if (checkDuplicateUserName.Status == (int)ProfileStatus.Active)
					{
						return BadRequest("profile with this user name already exist");
					}
					if (checkDuplicateUserName.Status == (int)ProfileStatus.Deactivated)
					{
						return BadRequest("profile with this user name exist but is deactivated");
					}
				}

				var checkTempDuplicate = UnitOfWork.TemCorporateProfileRepo.CheckDuplicateUserName(payload.UserName);
				if (checkTempDuplicate != null)
				{
					if (checkTempDuplicate.Status == (int)ProfileStatus.Pending)
					{
						return BadRequest("profile with this user name is awaiting approval");
					}
					if (checkDuplicateUserName.Status == (int)ProfileStatus.Active)
				...
				var auditTrail = new TblAuditTrail
				{
					...
					HostName = payload.HostName,
					ClientStaffIpaddress = payload.ClientStaffIPAddress,
					NewFieldValue = $"CorporateShortName : ${corporateCustomerDto.CorporateShortName}, Company Name: {corporateCustomerDto.CompanyName}, Customer ID: {corporateCustomerDto.CustomerId}, First Name: {entity.FirstName},Last Name: {entity.LastName}, Status: {nameof(ProfileStatus.Modified)}",
					PreviousFieldValue = $"CorporateShortName : ${corporateCustomerDto.CorporateShortName}, Company Name: {corporateCustomerDto.CompanyName}, Customer ID: {corporateCustomerDto.CustomerId}, First Name: {entity.FirstName}, Last Name: {entity.LastName} Status: {status}",
					TransactionId = "",
					UserId = BankProfile.Id,
					Username = UserName,
					Description = $"Updated Corporate Profile Role. Action was carried out by a Bank user",
				...
				mapTempProfile.PreviousStatus = entity.Status;
				mapTempProfile.Status = (int)ProfileStatus.Modified;
				mapTempProfile.Action = nameof(TempTableAction.Update_User_Name).Replace("_", " ");
				entity.Status = originalStatus;
				UnitOfWork.CorporateProfileRepo.UpdateCorporateProfile(entity);
				UnitOfWork.TemCorporateProfileRepo.Add(mapTempProfile);
				UnitOfWork.AuditTrialRepo.Add(auditTrail);
		...
		[ProducesResponseType(StatusCodes.Status200OK)]
		public async Task<ActionResult<bool>> ResetUserPassword(GenericRequestDto model)
		{
			try
			{
				if (!IsAuthenticated)
				...
				if (string.IsNullOrEmpty(UserRoleId) || !UnitOfWork.UserRoleAccessRepo.AccessesExist(UserRoleId, Permission.ResetUserPassword))
				{
					return BadRequest("UnAuthorized Access");
				}
				...
				mapTempProfile.PreviousStatus = entity.Status;
				mapTempProfile.Status = (int)ProfileStatus.Modified;
				mapTempProfile.Action = nameof(TempTableAction.Password_Reset).Replace("_", " ");
				UnitOfWork.TemCorporateProfileRepo.Add(mapTempProfile);
				UnitOfWork.AuditTrialRepo.Add(auditTrail);
				UnitOfWork.Complete();
				// return response
		...
		[HttpPost("PinReset")]
		[ProducesResponseType(StatusCodes.Status200OK)]
		public ActionResult<bool> ResetPin(GenericRequestDto model)
		{
			try
			{
				if (!IsAuthenticated)
				...
				{
					Id = Guid.NewGuid(),
					ActionCarriedOut = nameof(AuditTrailAction.Pin_Reset).Replace("_", " "),
					Ipaddress = payload.IPAddress,
					Macaddress = payload.MACAddress,
					HostName = payload.HostName,
					ClientStaffIpaddress = payload.ClientStaffIPAddress,
					NewFieldValue = $"Company Name: {corporateCustomerDto.CompanyName}, Customer ID: {corporateCustomerDto.CustomerId}, Pin Reset Initiated: True, Username: {entity.Username}, First Name: {entity.FirstName}, Last Name: {entity.LastName}",
					PreviousFieldValue = $"Company Name: {corporateCustomerDto.CompanyName}, Customer ID: {corporateCustomerDto.CustomerId}, First Name: {entity.FirstName}, " +
						$"Last Name: {entity.LastName}, Username: {entity.Username}, Email Address:  {entity.Email}, " + $"Middle Name: {entity.MiddleName}, Phone Number: {entity.Phone1}" + $"Approval Limit: {entity.ApprovalLimit}, Status: {status}",
					TransactionId = "",
					UserId = BankProfile.Id,
					Username = UserName,
					Description = $"Triggered Pin reset for Corporate User's Profile. Action was carried out by a Bank user",
					TimeStamp = DateTime.Now
				};
				...
				mapTempProfile.PreviousStatus = entity.Status;
				mapTempProfile.Status = (int)ProfileStatus.Modified;
				mapTempProfile.Action = nameof(TempTableAction.Pin_Reset).Replace("_", " ");
				var defaultPin = Generate16DigitNumber.GenerateRestPin();
				mapTempProfile.DefaultPin = Encryption.EncryptPin(defaultPin);
				UnitOfWork.TemCorporateProfileRepo.Add(mapTempProfile);
				UnitOfWork.AuditTrialRepo.Add(auditTrail);
				UnitOfWork.Complete();
				// return response
		...
		[ProducesResponseType(StatusCodes.Status200OK)]
		public async Task<ActionResult<bool>> ResendCorporateProfilePassword(GenericRequestDto model)
		{
			try
			{
				if (!IsAuthenticated)
				...
				{
					var path = Path.Combine(Template.ResendProfileDetails);
					await _emailService.SendEmail(EmailTemplate.ResendCustomerUserNameMail(entity.Email, fullName, entity.Username, path));
					_logger.LogInformation("Triggered Resent password for Corporate User's Profile {0}", JsonConvert.SerializeObject(entity.Email));
				}
				else
				{
					var path = Path.Combine(Template.CustomerProfileOnbording);
					await _emailService.SendEmail(EmailTemplate.UserOnboardingMailWithCredentialMail(entity.Email, fullName, entity.Username, password, path));
					_logger.LogInformation("Triggered Resent password for Corporate User's Profile {0}", JsonConvert.SerializeObject(entity.Email));
		...
		{
			if (requestInfo.Status != (int)ProfileStatus.Pending)
			{
				errorMessage = "this request is not pending approval";
				return false;
			}
			...
			if (requestInfo.Action == nameof(TempTableAction.Create).Replace("_", " "))
			{
				...
				mapProfile.PasswordExpiryDate = DateTime.Now.AddHours(24);
				mapProfile.ResetPinInitiated = 1;
			...
			if (requestInfo.Action == nameof(TempTableAction.Update).Replace("_", " "))
			{
				...
				bool canSendUser = false;
				if (entity.Username != requestInfo.Username)
				{
					canSendUser = true;
				}
				...
				entity.Username = requestInfo.Username;
				...
				if (canSendUser)
				{
					...
					ThreadPool.QueueUserWorkItem(_ => _emailService.SendEmail(EmailTemplate.ResendCustomerUserNameMail(entity.Email, fullName, requestInfo.Username, path)));
				}
				errorMessage = "ok";
				return true;
			}
			...
			if (requestInfo.Action == nameof(TempTableAction.Update_User_Name).Replace("_", " "))
			{
				var entity = UnitOfWork.CorporateProfileRepo.GetByIdAsync((Guid)requestInfo.CorporateProfileId);
				var auditTrail = new TblAuditTrail
				{
					...
					UserId = BankProfile.Id,
					Username = UserName,
					Description = $"Approved Corporate UserName Update. Action was carried out by a Bank user",
					TimeStamp = DateTime.Now
				};
				...
				var path = Path.Combine(Template.ResendUserName);
				_emailService.SendEmail(EmailTemplate.NewUserNameMail(entity.Email, entity.FullName, entity.Username, path));
				errorMessage = "";
				return true;
			}
			...
			if (requestInfo.Action == nameof(TempTableAction.Pin_Reset).Replace("_", " "))
			{

				if (string.IsNullOrEmpty(UserRoleId) || !UnitOfWork.UserRoleAccessRepo.AccessesExist(UserRoleId, Permission.CanApprovedTransactionPinReset))
				{
					errorMessage = "UnAuthorized Acces";
					return true;
				}
				var entity = UnitOfWork.CorporateProfileRepo.GetByIdAsync((Guid)requestInfo.CorporateProfileId);
				if (entity == null)
				...
				var auditTrail = new TblAuditTrail
				{
					...
					UserId = BankProfile.Id,
					Username = UserName,
					Description = $"Approved Corporate Profile Pin Reset. Action was carried out by a Bank user",
					TimeStamp = DateTime.Now
				};
				//bool resetInitiated = entity.ResetInitiated == 1 ? true : false;
				var path = Path.Combine(Template.ResetPin);
				var defaultPin = Encryption.DecriptPin(requestInfo.DefaultPin);
				string fullName = entity.LastName + " " + entity.MiddleName + " " + entity.FirstName;
				ThreadPool.QueueUserWorkItem(_ => _emailService.SendEmail(EmailTemplate.ResetPinCredentialMail(entity.Email, fullName, defaultPin, path)));

				entity.ResetPinInitiated = (int)ProfileStatus.Active;
				entity.DateResetPinInitiated = DateTime.Now;
				entity.ReasonsForDeactivation = "";
				entity.DefaultTranPin = requestInfo.DefaultPin;
				entity.PasswordExpiryDate = DateTime.Now.AddHours(24);
				requestInfo.Status = (int)ProfileStatus.Active;
				requestInfo.IsTreated = (int)ProfileStatus.Active;
				requestInfo.ApprovedId = BankProfile.Id;
				requestInfo.ApprovalUsername = UserName;
			...
			if (requestInfo.Action == nameof(TempTableAction.Password_Reset).Replace("_", " "))
			{

				if (string.IsNullOrEmpty(UserRoleId) || !UnitOfWork.UserRoleAccessRepo.AccessesExist(UserRoleId, Permission.ApprovedResetCorporateUserPassword))
				{
					errorMessage = "UnAuthorized Acces";
					return true;
				}
				...
				var auditTrail = new TblAuditTrail
				{
					...
					UserId = BankProfile.Id,
					Username = UserName,
					Description = $"Approved Corporate Profile Password Reset. Action was carried out by a Bank user",
					TimeStamp = DateTime.Now
				};
				//bool resetInitiated = entity.ResetInitiated == 1 ? true : false;
				...
				UnitOfWork.AuditTrialRepo.Add(auditTrail);
				UnitOfWork.Complete();
				var path = Path.Combine(Template.ResetPassword);
				string fullName = entity.LastName + " " + entity.MiddleName + " " + entity.FirstName;
				ThreadPool.QueueUserWorkItem(_ => _emailService.SendEmail(EmailTemplate.ResetPasswordCredentialMail(entity.Email, fullName, password, path)));
				errorMessage = "";
				return true;
			}
		...
		private bool RequestApproval(TblTempCorporateProfile entity, SimpleAction payload, out string errorMessage)
		{
			...
			if (entity.Action == nameof(TempTableAction.Update_User_Name).Replace("_", " "))
			{
				...
				var auditTrail = new TblAuditTrail
				{
					...
					HostName = payload.HostName,
					ClientStaffIpaddress = payload.ClientStaffIPAddress,
					NewFieldValue = $"CorporateShortName: {corporateCustomerDto.CorporateShortName} Company Name: {corporateCustomerDto.CompanyName}, Customer ID: {corporateCustomerDto.CustomerId}, First Name: {entity.FirstName}, Last Name: {entity.LastName}, Username: {entity.Username}, Email Address:  {entity.Email}, Middle Name: {entity.MiddleName}, Phone Number: {entity.Phone1}, Approval Limit: {entity.ApprovalLimit}, Status: {nameof(ProfileStatus.Pending)}",
					PreviousFieldValue = "",
					TransactionId = "",
					UserId = BankProfile.Id,
					Username = UserName,
					Description = $"Request Approval for Corporate Profile UserName Change. Action was carried out by a Bank user",
					TimeStamp = DateTime.Now
				};
			...
			if (entity.Action == nameof(TempTableAction.Pin_Reset).Replace("_", " "))
			{
				...
				var auditTrail = new TblAuditTrail
				{
					...
					UserId = BankProfile.Id,
					Username = UserName,
					Description = $"Request Approval for Corporate Profile Pin Reset. Action was carried out by a Bank user",
					TimeStamp = DateTime.Now
				};
			...
			if (entity.Action == nameof(TempTableAction.Reactivate).Replace("_", " "))
			{
				...
				var auditTrail = new TblAuditTrail
				{
					...
					UserId = BankProfile.Id,
					Username = UserName,
					Description = $"Request Approval for Corporate Profile Reactiviation. Action was carried out by a Bank user",
					TimeStamp = DateTime.Now
				};
				entity.Status = (int)ProfileStatus.Pending;
				UnitOfWork.TemCorporateProfileRepo.UpdateTempCorporateProfile(entity);
			...
			if (entity.Action == nameof(TempTableAction.Password_Reset).Replace("_", " "))
			{
				var profile = UnitOfWork.CorporateProfileRepo.GetByIdAsync((Guid)entity.CorporateProfileId);
				if (entity.Status != (int)ProfileStatus.Pending && entity.Status != (int)ProfileStatus.Declined && entity.Status != (int)ProfileStatus.Modified)
				{
		...
		private bool DeclineRequest(TblTempCorporateProfile entity, RequestCorporateProfileDto payload, out string errorMessage)
		{
			...
			if (entity.Action == nameof(TempTableAction.Pin_Reset).Replace("_", " "))
			{
				...
				var auditTrail = new TblAuditTrail
				{
					...
					UserId = BankProfile.Id,
					Username = UserName,
					Description = $"Decline Approval for  Corporate Profile Pin Reset. Action was carried out by a Bank user",
					TimeStamp = DateTime.Now
				};
				entity.Status = (int)ProfileStatus.Declined;
				profile.Status = (int)entity.PreviousStatus;
			...
			if (entity.Action == nameof(TempTableAction.Password_Reset).Replace("_", " "))
			{
				...
				var auditTrail = new TblAuditTrail
				{
					...
					UserId = BankProfile.Id,
					Username = UserName,
					Description = $"Decline Approval for  Corporate Profile Password Reset. Action was carried out by a Bank user",
					TimeStamp = DateTime.Now
				};
