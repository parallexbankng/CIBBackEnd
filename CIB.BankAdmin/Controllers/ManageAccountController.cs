...
namespace CIB.BankAdmin.Controllers
{
	...
	public class ManageAccountController : BaseAPIController
	{
		...
		[HttpGet("RelatedCustomerAccount")]
		[ProducesResponseType(StatusCodes.Status201Created)]
		public async Task<ActionResult<ResponseDTO<CustomerDataResponseDto>>> RelatedCustomerAccount(string customerid)
		{
			try
			{
				if (!IsAuthenticated)
				...
				//call name inquiry API
				var number = customerid.Trim();
				var accountInfo = await _apiService.RelatedCustomerAccountDetails(customerid);
				if (accountInfo.RespondCode != "00")
				{
					return BadRequest(accountInfo.RespondMessage);
				}
		...
		public ActionResult<bool> ValidateCorporateCustomerModel(GenericRequestDto model)
		{
			try
			{
				...
				var tempEntity = UnitOfWork.TemCorporateCustomerRepo.GetCorporateCustomerByCustomerIDForOnboarding(payload.CustomerId, nameof(TempTableAction.Onboard_Corporate_Customer).Replace("_", " "));
				if (tempEntity != null)
				{
					if (tempEntity.IsTreated == (int)ProfileStatus.Pending)
					{
						return BadRequest("Customer with the same customer Id is awaiting approval");
					}
					if (tempEntity.IsTreated == (int)ProfileStatus.Active)
					{
						return BadRequest("Customer with the same customer Id already exist");
					}
				}
			...
			try
			{
				...
				var auditTrail = new TblAuditTrail
				{
					...
					NewFieldValue = $"Company Name: {payload.CompanyName}, Customer ID: {payload.CustomerId}, " +
						...
						$"Admin Middle Name: {payload.MiddleName}, Admin Phone Number: {payload.PhoneNumber}, Admin Role: {role}, Admin Approval Limit {payload.ApprovalLimit}",
					PreviousFieldValue = "",
					TransactionId = "",
				...
				var temCustomer = new TblTempCorporateCustomer
				{
					...
					CorporateShortName = payload.CorporateShortName,
					CorporateRoleId = role != "" ? payload.CorporateRoleId : null,
					...
					AuthenticationLimit = payload.AuthenticationLimit,
					DefaultPin = payload.AuthenticationLimit != null ? Encryption.EncryptPin(Generate16DigitNumber.GenerateRestPin()) : null,
		...
		public async Task<ActionResult<bool>> BulkOnboardCorporateCustomer([FromForm] BulkOnboardCorporateCustomerRequestDto model)
		{
			try
			{
							...
							var task = Task.Run(() =>
											{
												var corporateCustomer = new TblCorporateCustomer
												{
													...
													CorporateShortName = row.CorporateShortName,
				...
				foreach (var profile in corporateProfiles)
				{
					...
					await _emailService.SendEmail(EmailTemplate.UserOnboardingMailWithCredentialMail(profile.Email, profile.FullName, profile.Username, password, path));
				}
				if (duplicate.Any())
				{
					return Ok(duplicate);
		...
		public ActionResult<bool> ChangeCorporateCustomerSignatory(GenericRequestDto model)
		{
			try
			{
				...
				var auditTrail = new TblAuditTrail
				{
					...
					Macaddress = payload.MACAddress,
					HostName = payload.HostName,
					NewFieldValue = $"Company Name: {corporateCustomer.CompanyName}, Customer ID: {corporateCustomer.CustomerId}, " +
					$"Authorization Type: {payload.AuthorizationType.Replace("_", " ")}, Default Account Name: {corporateCustomer.DefaultAccountName}, " +
					$"Default Account Number: {corporateCustomer.DefaultAccountNumber}, Email: {corporateCustomer.Email1}, Status: {nameof(ProfileStatus.Pending)}",
					PreviousFieldValue = $"Company Name: {corporateCustomer.CompanyName}, Customer ID: {corporateCustomer.CustomerId}, " +
					$"Authorization Type: {corporateCustomer.AuthorizationType.Replace("_", " ")}, Default Account Name: {corporateCustomer.DefaultAccountName}, " +
					$"Default Account Number: {corporateCustomer.DefaultAccountNumber}, Email: {corporateCustomer.Email1}",
					TransactionId = "",
					UserId = BankProfile.Id,
					Username = UserName,
					Description = "Change Account Signatory by Bank User",
					TimeStamp = DateTime.Now
				};
				UnitOfWork.AuditTrialRepo.Add(auditTrail);
