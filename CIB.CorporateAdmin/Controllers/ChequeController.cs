...
namespace CIB.CorporateAdmin.Controllers
{
    ...
    public class ChequeController : BaseAPIController
    {
        ...
        public ChequeController(ILogger<ChequeController> logger,IUnitOfWork unitOfWork, IMapper mapper, IHttpContextAccessor accessor) : base( unitOfWork, mapper,accessor)
        {
            _logger = logger;
        }
        ...
        public ActionResult<ResponseDTO<RequestChequeBookDto>> CreateCorporateProfile(RequestChequeBookDto model)
        {
            try
            {
                ...
                //     NumberOfLeave = model.NumberOfLeave,
                //     CorporateCustomerId = model.CorporateCustomerId,
                //     IPAddress = model.IPAddress,
                //     HostName = model.HostName,
                //     ClientStaffIPAddress = model.ClientStaffIPAddress,
        ...
        [ProducesResponseType(StatusCodes.Status201Created)]
        public ActionResult<ListResponseDTO<RequestChequeBookHistory>> CreateCorporateProfile()
        {
            try
            {
                if (!IsAuthenticated)
                ...
                var corporateCustomerDto =  UnitOfWork.CorporateCustomerRepo.GetByIdAsync(CorporateProfile.CorporateCustomerId.Value);
                if (corporateCustomerDto == null)
                {
                    return BadRequest("Invalid Corporate Customer ID");
                }
